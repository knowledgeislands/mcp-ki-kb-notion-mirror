...
- `NODE_ENV` is set to `development` only by `ki:server:mcp:dev` and `ki:server:mcp:inspect`. Claude Desktop doesn't set it, so `.env.*` is ignored in production — `MCP_KB_NOTION_MIRROR_TOKEN` must come from the Claude Desktop config `env` block.
