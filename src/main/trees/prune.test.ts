...
describe('prune', () => {
  ...
  it('dry-run plans the committed and working-tree orphans, ignoring moves and no-url deletions', async () => {
    ...
    expect(
      res.outcomes
        .map((o) => ({ kbPath: o.kbPath, action: o.action, url: o.url }))
        .sort((a, b) => a.kbPath.localeCompare(b.kbPath))
    ).toEqual([
  ...
  it('ignores live urls under node_modules and below MAX_WALK_DEPTH when deciding orphans', async () => {
    ...
    const deepRel = Array.from({ length: MAX_WALK_DEPTH + 2 }, (_, i) => `D${i}`).reduce(
  ...
  it('pruneRoots scans the whole KB and archives orphans when not a dry run', async () => {
    ...
    expect(archived).toEqual(
      expect.arrayContaining([
        `https://api.notion.test/v1/pages/${hex(2)}`,
        `https://api.notion.test/v1/pages/${hex(3)}`
      ])
    )
  })
