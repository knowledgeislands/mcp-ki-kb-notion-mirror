...
If you find a security issue in `@knowledgeislands/mcp-ki-kb-notion-mirror`, **please do not file a public GitHub issue.** Instead, email the maintainer directly:

- **<kris@kris.me.uk>** — subject: `mcp-ki-kb-notion-mirror security`
...
- The version of the package (`npm ls @knowledgeislands/mcp-ki-kb-notion-mirror`) and Node version.
...
`mcp-ki-kb-notion-mirror` is a stdio MCP server that holds a Notion internal-integration secret in `MCP_KI_KB_NOTION_MIRROR_TOKEN`, walks a user-supplied KB filesystem subtree, creates pages in Notion, and writes back to local KB notes. It runs locally with the privileges of the user who launched it. The security boundary is the token (full read/insert/update access to every page the integration is connected to) and the local filesystem under `MCP_KI_KB_NOTION_MIRROR_KB_ROOT`.
...
4. **Destructive tools require `dry_run: true` by default.** `notion_mirror_unpublish` and `notion_mirror_tree_publish` neither call Notion nor edit notes unless `dry_run` is explicitly `false`. The `destructive` access level is opt-in via `MCP_KI_KB_NOTION_MIRROR_ACCESS_LEVEL`.
